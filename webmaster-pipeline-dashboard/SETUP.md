...
**«Торг» и дата в калькуляторе:** по умолчанию **`Комментарий (Денис)`** (`COL_TRADE_DATE`). Пустой `COL_TRADE_DATE` — авто: сначала комментарий Дениса, иначе столбец с «дата». **`TRADE_DATE_MAX_AGE_DAYS`** (по умолчанию 30): окно дат назад от сегодня; **0** = только сегодня. Убедитесь, что **`GID_CALCULATOR_TAB_1`** = вкладка с данными (часто **Telecomasia**).
