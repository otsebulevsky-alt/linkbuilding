...
**«Торг» и дата в калькуляторе:** по умолчанию колонка **`Комментарий (Денис)`** (`COL_TRADE_DATE`); при другом заголовке — задать в Secrets. Пустой `COL_TRADE_DATE` — авто по слову «дата» в шапке листа.
