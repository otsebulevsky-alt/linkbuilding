...
2. **Segfault / malloc / `Aborted` сразу после «Processed dependencies»** (в логах: **Signal 11**, **`Segmentation fault`**, **`run-streamlit.sh` … Aborted**): (а) Корневой [`runtime.txt`](../runtime.txt) = **`python-3.11.10`** и в UI — **Python 3.11**. (б) Один корневой [`requirements.txt`](../requirements.txt): **streamlit** и **watchdog** пинятся явно; **pyarrow** в дашборд не входит (нет Parquet — меньше нативных `.so` на Linux). (в) В [корневом `.streamlit/config.toml`](../.streamlit/config.toml) и в [`webmaster-pipeline-dashboard/.streamlit/config.toml`](.streamlit/config.toml) задано **`[server] fileWatcherType = "none"`** — watcher на Cloud не нужен и чаще всего безопаснее, чем `poll`. (г) Без второго `requirements.txt` и без **`-r ...`**. (д) **Main file path** = **`webmaster-pipeline-dashboard/app.py`**, **App root** пустой (или корневой шим `app.py` — см. этап F).
