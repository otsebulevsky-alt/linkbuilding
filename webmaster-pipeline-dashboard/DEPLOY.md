...
3. **Проверка, что Cloud подтянул свежий код:** в серой подписи под заголовком «Панель линкбилдинга» в начале должна быть метка вида **`mail-ui-2026-04-01c`**. **«Сменить почту»** — в **сайдбаре**: открывается поле для **адреса**; пароль приложения и SMTP/IMAP-серверы остаются из **Secrets** (подмена только на время сессии браузера). В основном ряду первая синяя кнопка — **«Прочитать почту»**. Если метки нет — закоммитьте/push в GitHub-репозиторий приложения и сделайте **Reboot app** в Streamlit.
