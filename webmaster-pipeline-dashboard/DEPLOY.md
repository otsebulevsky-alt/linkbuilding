...
3. **Проверка, что Cloud подтянул свежий код:** в серой подписи под заголовком «Панель линкбилдинга» в начале должна быть метка вида **`mail-ui-2026-04-01f`**. **«Сменить почту»** — **справа от заголовка** «Панель линкбилдинга»; открывается поле **адреса** (пароль и серверы — из **Secrets**, подмена на время сессии). Ниже — четыре синие кнопки действий. В сайдбаре только статус почты. Если метки нет — закоммитьте/push в GitHub-репозиторий приложения и сделайте **Reboot app** в Streamlit.
