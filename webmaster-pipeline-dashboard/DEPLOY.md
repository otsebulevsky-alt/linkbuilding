...
3. **Проверка, что Cloud подтянул свежий код:** в серой подписи под заголовком «Панель линкбилдинга» в начале должна быть метка вида **`mail-ui-2026-04-01d`**. **«Сменить почту»** — **первая** из пяти синих кнопок в основной области; открывается поле **адреса** (пароль и серверы — из **Secrets**, подмена на время сессии). В сайдбаре только статус почты. Если метки нет — закоммитьте/push в GitHub-репозиторий приложения и сделайте **Reboot app** в Streamlit.
