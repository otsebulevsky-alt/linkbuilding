...
2. **Segfault / malloc / `Aborted` сразу после «Processed dependencies»** (в логах: **Signal 11**, **`Segmentation fault`**, **`run-streamlit.sh` … Aborted**): (а) Корневой [`runtime.txt`](../runtime.txt) = **`python-3.11.10`** и в UI — **Python 3.11**. (б) Один корневой [`requirements.txt`](../requirements.txt): **streamlit** пинится; **отдельно не пинить watchdog** (конфликт версий с зависимостью Streamlit + **uv** → дубли `.so`); **pyarrow** в дашборд не входит. В коде до `pandas` / Google API: **`PROTOCOL_BUFFERS_PYTHON_IMPLEMENTATION=python`** и **`MALLOC_ARENA_MAX=2`** (см. корневой [`app.py`](../app.py) и [`webmaster-pipeline-dashboard/app.py`](app.py)). Дополнительно в **Settings → Advanced settings** можно задать те же переменные вручную. (в) В [корневом `.streamlit/config.toml`](../.streamlit/config.toml) и в [`webmaster-pipeline-dashboard/.streamlit/config.toml`](.streamlit/config.toml) — **`[server] fileWatcherType = "none"`**. (г) Без второго `requirements.txt` и без **`-r ...`**. (д) **Main file path** = **`webmaster-pipeline-dashboard/app.py`**, **App root** пустой (или корневой шим `app.py` — см. этап F).
