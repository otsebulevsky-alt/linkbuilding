...
После успешного обновления кода в подписи под заголовком панели должна появиться метка из константы **`PANEL_UI_BUILD`** в `webmaster-pipeline-dashboard/app.py` (например **`panel-2026-04-03-inbox-cleanup-safe`**).
...
3. **Проверка, что Cloud подтянул свежий код:** в серой подписи под заголовком «Панель линкбилдинга» в начале должна быть метка **`PANEL_UI_BUILD`** из `app.py` (например **`panel-2026-04-03-inbox-cleanup-safe`**). **«Сменить почту»** — **справа от заголовка**; в форме — **логин** и при необходимости **пароль приложения** (можно только на сессию, без Secrets). Ниже — четыре синие кнопки. Если метки нет — push на GitHub и **Reboot app**.
...
## Если из «Сбор с ответов» пропали строки после очистки в панели

1. В Google Таблицах: **Файл → История версий → Смотреть историю версий** — восстановите версию **до** нажатия «Удалить такие строки из листа».
2. В актуальном коде панели правило «только email в ячейке Почта = шум» **отключено** (после IMAP-синка в «Почте» часто один адрес — иначе удалялись все строки). При массовом удалении (≥75% строк) операция **блокируется**.
