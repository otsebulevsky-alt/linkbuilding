...
1. **`GID_CALCULATOR_TAB_1`** в Secrets = число **`gid=`** из URL **той же вкладки**, где реально таблица с **Domain / Цена / Ответственный** (вкладки **Telecomasia** и **Telecomesia** или «Калькулятор» — разные `gid`; ошибка «не сопоставлены колонки» часто из‑за неверного листа).
...
**Последнее обновление:** 2026-03-30 (Торг: чтение калькулятора **A1:ZZ10000**, **COL_TRADE_RESPONSIBLE**; Cloud: Python **3.11**, **pyarrow 14.x**; метка `PANEL_UI_BUILD` в `app.py`)
