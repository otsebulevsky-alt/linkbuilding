...
2. **Segfault / malloc / `Aborted` сразу после «Processed dependencies»** (в логах: **Signal 11**, **`Segmentation fault`**, **`run-streamlit.sh` … Aborted**): (а) Корневой [`runtime.txt`](../runtime.txt) = **`python-3.11.10`** и в UI — **Python 3.11**. (б) Один корневой [`requirements.txt`](../requirements.txt): **streamlit** пинится; **обязательно пинятся `pyarrow`, `tornado`, `watchdog`, `pillow`** — **Streamlit сам тянет `pyarrow>=7`**; без пинов **uv** часто ставит свежие колёса → **SIGSEGV** на части инстансов. (в) В **Secrets** (TOML) добавьте строки из [secrets.toml.example](secrets.toml.example) блока про Cloud: **`PROTOCOL_BUFFERS_PYTHON_IMPLEMENTATION`**, **`MALLOC_ARENA_MAX`**, при необходимости **`OMP_NUM_THREADS`** / **`OPENBLAS_NUM_THREADS`** — они попадают в **os.environ** раньше импортов. Плюс в коде: корневой [`app.py`](../app.py) и [`webmaster-pipeline-dashboard/app.py`](app.py). (г) В [корневом `.streamlit/config.toml`](../.streamlit/config.toml) и в [`webmaster-pipeline-dashboard/.streamlit/config.toml`](.streamlit/config.toml) — **`[server] fileWatcherType = "none"`**. (д) Без второго `requirements.txt` и без **`-r ...`**. (е) **Main file path** = **`webmaster-pipeline-dashboard/app.py`**, **App root** пустой (или корневой шим `app.py` — см. этап F).
