"""Кнопка «торг» (как в процессе линкбилдинга):

1. **Калькулятор доноров** — строки за сегодня по ответственному: домен (кол. A), цена (кол. H).
2. **«Сбор с ответов»** — по домену ищется строка, из ячейки «Почта» извлекается email вебмастера.
3. **IMAP** — в ящиках INBOX → **Отправленные** (\\Sent) → **Вся почта** (\\All из LIST) ищется переписка
   с этим адресом и упоминанием домена; от неё берётся тред (Message-ID / In-Reply-To, References).
4. **SMTP** — в **тот же тред** уходит HTML с предложением снизить цену на TRADE_DISCOUNT_PERCENT (по умолчанию 20%).
"""
