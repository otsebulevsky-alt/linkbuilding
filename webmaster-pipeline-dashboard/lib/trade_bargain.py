...
def run_trade_bargain_round(
    *,
    ...
    for nd, pack in sorted(by_domain.items(), key=lambda x: x[0]):
        ...
        body = (
            f"<p>Hello,</p>"
            f"<p>Thank you. We would really love to work with you on <b>{esc_dom}</b>. "
            f"Could we agree on a small <b>{pct}% discount</b>? "
            f"That would be <b>{esc_new}</b> USD instead of <b>{esc_price}</b> USD. "
            f"If that works for you, we would be glad to proceed.</p>"
            f"<p>Kind regards</p>"
            f"<p>—</p>"
            f"<p>Здравствуйте.</p>"
