...
PANEL_UI_BUILD = "panel-2026-04-03-imap-multi-search-sender-irt"
...
def main():
    ...
    if st.session_state.imap_sync_report is not None:
        ...
        with st.expander("📬 Синхронизация ответов вебмастеров → таблица «Сбор с ответов»", expanded=True):
            st.caption(
                "**Как отсеиваются «не ответы»:** только на этапе этой кнопки — шум **не записывается** в таблицу "
                "(письмо помечается прочитанным). В лист попадают строки только там, где есть домен вебмастера и письмо "
                "**не** попало под фильтры ниже. **Массового удаления строк с листа в панели нет** — ответы в уже "
                "записанных строках не трогаются автоматически. "
