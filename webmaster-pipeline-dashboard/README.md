...
### Как задумывалось в документах HACK-382

Первоисточник: [implementation-notes](../../../../shared-docs/events/2026-03-30_ai-hackaton/results/hack-382/implementation-notes.md) (раздел **«Вариант E — Streamlit»**), кратко [README hack-382](../../../../shared-docs/events/2026-03-30_ai-hackaton/results/hack-382/README.md). Панель = **Python Streamlit** + ключи в **Secrets**; для стадии 4 хакатона — **GitLab CI** ([`../.gitlab-ci.yml`](../.gitlab-ci.yml)): юнит-тесты дашборда и зеркало ветки в **GitHub**, откуда [Streamlit Community Cloud](https://streamlit.io/cloud) подтягивает код (см. [DEPLOY.md](DEPLOY.md)).
...
**Постоянный URL в интернете (24/7):** [https://webmaster-pipeline-dashboard.streamlit.app/](https://webmaster-pipeline-dashboard.streamlit.app/) — см. также [DEPLOY.md](DEPLOY.md) (GitLab CI → GitHub → Streamlit Cloud). Для работы только с ПК облако не обязательно.
...
3. Таблица **«Сбор с ответов»** (`SPREADSHEET_INBOX_LOG_ID`, по умолчанию книга с листом для ответов вебмастеров): для кнопки **«Прочитать почту»** (запись строк из IMAP) сервисному аккаунту нужна роль **Редактор** на эту книгу. Первая строка листа: `Домен`, `дата`, `цена`, `Цена после торг`, `Почта`, и при необходимости **шестой столбец** для черновика про оплату (например `Черновик ответа (оплата)`) — его заполняет синхрон, если ответ вебмастера не согласуется со справочником «Возможности оплаты» или в письме нет вариантов оплаты.
4. Убедитесь, что первая строка **реестров** содержит заголовки: `Linkbuilder`, `Status`, `Article/post`, `Website Donor`, `Cost $` — как в конфиге или поправьте ключи в `secrets.toml`.
...
3. Вставьте в `GMAIL_SMTP_USER`, `GMAIL_SMTP_APP_PASSWORD` (для рабочего сценария — **`o.tsebulevsky@rantsports.com`** и пароль **приложения**; те же значения можно указать в `GMAIL_IMAP_*` или оставить только SMTP — кнопка «Прочитать почту» подставит их).

Без этого кнопка «Отправить письмо» и IMAP работать не будут; вкладки с таблицами — будут.

**Кнопка «Прочитать почту»:** IMAP `SEARCH UNSEEN` → для каждого письма извлекаются домены из темы (`for site.com`) и URL в теле, цена → в лист «Сбор с ответов» добавляется по строке на домен (колонки A–E). В колонку **дата** пишется **дата нажатия кнопки** (синхрон), в часовом поясе **`TRADE_TIMEZONE`** (по умолчанию `Europe/Moscow`), а не дата из заголовка письма. Письмо помечается прочитанным только если хотя бы одна строка успешно записана в таблицу. Раньше в коде был жёсткий лимит **40** писем за запуск (защита от долгого запроса на хостинге); по умолчанию теперь **без лимита** (`IMAP_SYNC_MAX_MESSAGES` не задан или **0** — обрабатываются все UNSEEN). Если нужен потолок, задайте в Secrets положительное число. При сотнях писем за один клик возможен таймаут Streamlit Cloud или обрыв IMAP — тогда увеличьте `IMAP_SYNC_TIMEOUT_SEC` или запускайте синхрон локально / делайте несколько нажатий после частичной обработки.

**Кнопка «отправить статьи»:** строки **«Жду публикации»**, в **Article/post** — **Google Docs / Drive**; домен **Website Donor**; почта из **«Сбор с ответов»**. Отправка — **ответ в тот же тред** (IMAP + In-Reply-To / References); без треда — пропуск `no_imap_thread`. Лимит **`ARTICLE_PUBLISH_MAX_SEND`**. Ручная отправка с вкладки «Жду публикации» — отдельное письмо без треда.

**Кнопка «проверка публикаций»:** строки со статусами из **`PUBLICATION_CHECK_STATUSES`** (по умолчанию только **Готово**; при необходимости добавьте **Жду публикации** в Secrets). В **Article/post** (колонка **H**) — URL поста; на HTML страницы проверяется одна ссылка `<a>`: **href** из колонки **Outgoing link** (**J**) и видимый текст = **Anchor** (**I**); нормализация URL как у href (www, хвостовой `/`). **EEAT** — **`EEAT_AUTHOR_MARKERS`** и колонки автора (как [apps-script-weekly-outreach-sync.md](../../../../shared-docs/seo/linkbuilding/context/apps-script-weekly-outreach-sync.md)). Заголовки колонок задаются **`COL_ANCHOR`**, **`COL_OUTGOING_LINK`**; пустой **`COL_OUTGOING_LINK`** → используется **`COL_PLACED_TARGET_URL`** (совместимость). **`PUBLICATION_CHECK_ANCHOR_CASE_INSENSITIVE`** — сравнение анкора без учёта регистра. **Индекс Google** — опционально **Custom Search JSON API** (`GOOGLE_CSE_API_KEY`, `GOOGLE_CSE_CX`).

**Как сочетаются «Прочитать почту», «торг» и статьи:** в колонке **дата** «Сбор с ответов» синхрон пишет **дату запуска** «Прочитать почту» в формате **`ГГГГ-ММ-ДД`** (тот же `TRADE_TIMEZONE`, что и для «сегодня» в торге). **«Торг»** и **«отправить статьи»** используют почту из последней строки по домену и шлют письмо **ответом в тред** (нужен IMAP). В **калькуляторе** дата для торга: **`COL_TRADE_DATE`** (по умолчанию **`Комментарий (Денис)`**). Пустой **`COL_TRADE_DATE`** — сначала колонка **«Комментарий» + «Денис»** в шапке, иначе первая подходящая колонка со словом **«дата»** (без «коммент»). Окно дат: **`TRADE_DATE_MAX_AGE_DAYS`** (по умолчанию **30** календарных дней до сегодня включительно; **0** = только сегодня). Форматы ячейки: **`ДД.ММ.ГГГГ`**, **`ГГГГ-ММ-ДД`**, **`ГГГГ ММ ДД`**. В отчёте «Торг» — **диагностика фильтра** и имя листа калькулятора. Порядок: **«Прочитать почту»** → при необходимости **«торг»** / **«отправить статьи»**; **«проверка публикаций»** — когда в **Article/post** уже стоит URL поста (не Google Docs).
