...
**Кнопка «отправить статьи»:** по всем вашим строкам со статусом **«Жду публикации»** в **обоих** реестрах ищется почта в **«Сбор с ответов»** по домену (**Website Donor**), как у «торг». Уходит **двуязычное** письмо (статья + сумма из реестра). Без ссылки в **Article/post** строка пропускается. Лимит писем за клик — **`ARTICLE_PUBLISH_MAX_SEND`** (по умолчанию 50; **0** = без лимита). Одна строка — одно письмо; ручное редактирование текста — вкладка **«Жду публикации → отправка»**.
