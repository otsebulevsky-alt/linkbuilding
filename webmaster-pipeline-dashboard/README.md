...
3. Таблица **«Сбор с ответов»** (`SPREADSHEET_INBOX_LOG_ID`, по умолчанию книга с листом для ответов вебмастеров): для кнопки **«Прочитать почту»** (запись строк из IMAP) сервисному аккаунту нужна роль **Редактор** на эту книгу. Первая строка листа: `Домен`, `дата`, `цена`, `Цена после торг`, `Почта` (как у вас в шаблоне).
