...
**Кнопка «отправить статьи»:** строки со статусом **«Жду публикации»** (колонка **Status**, у вас часто **E**) и со ссылкой на **Google Docs / Drive** в **Article/post** (часто **H**); домен из **Website Donor** (**A**). Почта — из **«Сбор с ответов»** по домену (как у «торг»). Текст письма: просьба разместить статью + ссылка на док; сумма из **Cost $**. Ссылка на уже опубликованный URL (не Docs) — строка пропускается (`not_google_doc_url`). Отправка — **новое** письмо, не ответ в тред. Лимит: **`ARTICLE_PUBLISH_MAX_SEND`** (по умолчанию 50; **0** = без лимита). Ручная отправка — вкладка **«Жду публикации → отправка»**.

**Как сочетаются «Прочитать почту» и «торг»:** в колонке **дата** «Сбор с ответов» синхрон пишет дату письма в формате **`ГГГГ-ММ-ДД`** (например `2026-04-02`). Кнопка **«торг»** при выборе «последней» строки по домену понимает и **`ГГГГ-ММ-ДД`**, и **`ДД.ММ.ГГГГ`**. В **калькуляторе** для фильтра «сегодня» используется только **`ДД.ММ.ГГГГ`** в отдельном столбце даты торга (`COL_TRADE_DATE` в Secrets при нескольких столбцах с «датой»). Рекомендуемый порядок: сначала **«Прочитать почту»** (чтобы в таблице появились **Домен** и **Почта**), затем **«торг»** для строк калькулятора за сегодня.
