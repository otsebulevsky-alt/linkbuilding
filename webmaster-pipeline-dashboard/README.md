...
**Кнопка «Прочитать почту»:** IMAP `SEARCH UNSEEN` → для каждого письма извлекаются домены из темы (`for site.com`) и URL в теле, цена и дата → в лист «Сбор с ответов» добавляется по строке на домен (колонки A–E). Письмо помечается прочитанным только если хотя бы одна строка успешно записана в таблицу. Раньше в коде был жёсткий лимит **40** писем за запуск (защита от долгого запроса на хостинге); по умолчанию теперь **без лимита** (`IMAP_SYNC_MAX_MESSAGES` не задан или **0** — обрабатываются все UNSEEN). Если нужен потолок, задайте в Secrets положительное число. При сотнях писем за один клик возможен таймаут Streamlit Cloud или обрыв IMAP — тогда увеличьте `IMAP_SYNC_TIMEOUT_SEC` или запускайте синхрон локально / делайте несколько нажатий после частичной обработки.
